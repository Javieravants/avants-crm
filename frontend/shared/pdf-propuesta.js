...
const PDFPropuesta = {
  ...
  generate(data) {
    ...
    const label = (text, x, yy) => { doc.setFontSize(7.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(...gray); doc.text(text.toUpperCase(), x, yy); };
    const val = (text, x, yy, bold) => { doc.setFontSize(10); doc.setFont('helvetica', bold ? 'bold' : 'normal'); doc.setTextColor(...black); doc.text(text || '-', x, yy); };
    const sectionTitle = (text, yy) => { doc.setFontSize(8); doc.setFont('helvetica', 'bold'); doc.setTextColor(...gray); doc.text(text.toUpperCase(), ML, yy); return yy + 5; };
    ...
    doc.text('Correduria de Seguros', ML, y + 5);
    ...
    doc.text(`Ref: ${ref}  ·  ${data.fecha || new Date().toLocaleDateString('es-ES')}`, ML + CW, y + 5, { align: 'right' });
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...black);
    doc.text(data.clienteNombre || 'Sin nombre', ML, y);
    y += 5;
    ...
    if (datos) { doc.text(datos, ML, y); y += 4; }
    if (data.clienteDireccion) { doc.text(data.clienteDireccion, ML, y); y += 4; }
    if (data.agente) {
      doc.text(`Agente: ${data.agente}`, ML, y);
      y += 4;
    }
    ...
    if (data.asegurados && data.asegurados.length > 0) {
      ...
      data.asegurados.forEach((a, i) => {
        doc.setFontSize(8.5);
        doc.text(`${i + 1}`, ML, y);
        doc.text((a.nombre || '-').substring(0, 35), ML + 8, y);
        doc.text(a.edad !== undefined ? `${a.edad} anos` : '-', ML + 90, y);
        doc.text(a.sexo || '-', ML + 108, y);
        doc.text(a.zona ? `Z${a.zona}` : '-', ML + 125, y);
        if (a.precio) {
          doc.setTextColor(...black);
          doc.text(`${a.precio} E`, ML + CW, y, { align: 'right' });
        }
        doc.setTextColor(...black);
        y += 5;
      });
    ...
    (data.productos || []).forEach(p => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...black);
      doc.text(p.nombre || 'Producto', ML, y);

      // Línea punteada hasta el precio
      const nameWidth = doc.getTextWidth(p.nombre || 'Producto');
      const priceStr = `${p.precioMensual || '0,00'} E/mes`;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...accent);
    doc.text(`${data.totalMensual || '0,00'} E`, ML + CW, y, { align: 'right' });
    y += 6;
    ...
    doc.setTextColor(...black);
    doc.setFont('helvetica', 'bold');
    doc.text(`${data.totalAnual || '0,00'} E`, ML + CW, y, { align: 'right' });
    ...
    if (data.descuentos) {
      ...
      if (descLine) { doc.text(descLine, ML, y); y += 4; }
    }
    ...
    if (data.puntos > 0) {
      y = sectionTitle('Campana MasProteccion', y);

      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...black);
      doc.text(`Puntos acumulados: ${data.puntos.toLocaleString('es-ES')} pts`, ML, y);
      y += 5;

      if (data.regalo) {
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...accent);
        doc.text(`Premio disponible: ${data.regalo}`, ML, y);
        y += 5;
      }

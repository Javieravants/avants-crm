...
const FichateModule = {
  ...
  async render() {
    ...
    if(!document.getElementById('ft-css')){
      ...
      st.textContent=`
        ...
        .ft-nb.a{background:#e6f5fc;color:#009DDD;font-weight:600}
        .ft-nb-b{margin-left:auto;background:#009DDD;color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;min-width:18px;text-align:center}
        ...
        .ft-bp{background:#009DDD;color:#fff}.ft-bg2{background:#22c55e;color:#fff}.ft-br{background:#ef4444;color:#fff}
        ...
        .ft-inp:focus{border-color:#009DDD;box-shadow:0 0 0 3px rgba(255,74,110,.08)}
    ...
    c.innerHTML=`<div id="ft-app">
      <aside class="ft-sb">
        <div style="padding:24px 22px;display:flex;align-items:center;gap:12px">
          <div style="width:38px;height:38px;border-radius:10px;background:#009DDD;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:900;color:#fff">F</div>
  ...
  async doClock(action){
    try{
      ...
      const m=d.action==='clock_in'?`<div class="ft-tg" style="background:#f0fdf4;color:#22c55e">✅ Entrada registrada: ${this.fT(d.time)}</div>`:`<div class="ft-tg" style="background:#e6f5fc;color:#009DDD">✅ Salida registrada: ${this.fT(d.time)}</div>`;
  ...
  dashH(){
    ...
    <div class="ft-sg">
      ...
      <div class="ft-cd ft-st"><div class="ft-st-l">Horas Hoy</div><div class="ft-st-v" style="color:#009DDD">${d.today_hours}h</div><div class="ft-st-s">Media: ${avgH}h por persona</div></div>
  ...
  clkH(){
    ...
    return `<div style="max-width:900px;margin:0 auto">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
        <div class="ft-cd" style="padding:18px 20px"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.8px">Vacaciones</div><div style="font-size:32px;font-weight:900;color:#009DDD;margin-top:4px">${myVac}<span style="font-size:14px;font-weight:500;color:#9ca3af"> días libres</span></div><div style="font-size:12px;color:#6b7280;margin-top:2px">${myUsed} usados de ${myTotal}</div></div><div style="width:56px;height:56px;border-radius:50%;background:conic-gradient(#009DDD ${Math.round((myUsed/myTotal)*100)}%, #f1f5f9 0);display:flex;align-items:center;justify-content:center"><div style="width:44px;height:44px;border-radius:50%;background:white;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#009DDD">${Math.round((myVac/myTotal)*100)}%</div></div></div></div>

        <div class="ft-cd" style="padding:18px 20px"><div style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px">Próximos festivos</div>${nextHols.length===0?'<div style="font-size:13px;color:#9ca3af">Sin festivos próximos</div>':nextHols.map(h=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #f8f8f8"><span style="font-size:13px;font-weight:600">${h.name||'Festivo'}</span><span style="font-size:12px;color:#009DDD;font-weight:600">${this.fD(typeof h.date==='object'?h.date.toISOString().split('T')[0]:h.date)}</span></div>`).join('')}</div>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
          <div class="ft-cd" style="padding:0;overflow:hidden">
            <div style="padding:32px 28px;background:linear-gradient(135deg,#009DDD,#ff7e95);color:#fff;text-align:center">
  ...
  profH(){
    ...
    <div class="ft-sg">
      <div class="ft-cd ft-st"><div class="ft-st-l">Vacaciones</div><div class="ft-st-v" style="color:#009DDD">${(e.vacation_days||22)-(e.used_vacation_days||0)}</div><div class="ft-st-s">${e.used_vacation_days||0} de ${e.vacation_days||22}</div></div>
  ...
  recH(){
    ...
    return `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:8px">
      ...
      <span style="font-weight:700;font-size:12px;color:#009DDD">${uniqueDays} días · ${r.length} fichajes · ${totalH.toFixed(1)}h</span>
    </div>
    <div class="ft-cd" style="padding:0;overflow:hidden">${rows.length===0?'<div class="ft-empty">Sin registros este mes</div>':`<table class="ft-tbl"><thead><tr><th>Fecha</th><th>Empleado</th><th>Entrada M</th><th>Salida M</th><th>Entrada T</th><th>Salida T</th><th style="text-align:right">Total</th>${this.isA()?'<th></th>':''}</tr></thead><tbody>${rows.map(g=>{
      ...
      return`<tr>
        ...
        ${this.isA()?`<td><button class="ft-btn ft-bs ft-bo" style="color:#009DDD;font-size:10px" onclick="FichateModule.editRecM(${s1.id||g.ids[0]})">✏️</button></td>`:''}
  ...
  reqCalH() {
    ...
    for (let d=1; d<=daysInMonth; d++) {
      ...
      dayHeaders += `<div style="text-align:center;min-width:28px;padding:2px 0;${isToday?'background:#009DDD;color:#fff;border-radius:6px;font-weight:800;':isHol?'color:#d97706;font-weight:700;':isWe?'color:#ccc;':'color:#6b7280;'}">
  ...
  credH(){
    ...
    const credRows=(creds,canEdit)=>creds.map(c=>`<tr><td style="font-weight:600">${this.esc(c.app_name)}${c.app_url?` <a href="${c.app_url}" target="_blank" style="font-size:11px;color:#009DDD">🔗</a>`:''}</td><td style="font-size:12px">${c.username||'—'}</td><td style="font-family:monospace;font-size:12px;font-weight:600">${c.password_plain||'—'}</td><td style="font-size:12px;color:#9ca3af">${c.notes||'—'}</td>${canEdit?`<td><div style="display:flex;gap:3px"><button class="ft-btn ft-bs ft-bo" style="color:#009DDD" onclick="FichateModule.editCredM(${c.id})">✏️</button><button class="ft-btn ft-bs ft-bo" style="color:#ef4444" onclick="FichateModule.delCred(${c.id})">✕</button></div></td>`:''}</tr>`).join('');

    const loginCard=me?`<div class="ft-cd ft-cd-p" style="margin-bottom:16px"><div style="font-weight:700;font-size:15px;margin-bottom:12px">🔐 Acceso Fichate</div><div style="background:#e6f5fc;border-radius:10px;padding:14px">${[['Email',me.email],['Contraseña',me.password_plain||'—']].map(([l,v])=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,74,110,.1)"><span style="font-size:13px;color:#6b7280">${l}</span><span style="font-size:13px;font-weight:700">${v}</span></div>`).join('')}</div></div>`:'';
  ...
  setH(){
    ...
    <div class="ft-cd ft-cd-p">
      ...
      ${turnos.length===0?'<div class="ft-empty">No hay turnos definidos</div>':`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px">${turnos.map(t=>`<div class="ft-cd ft-cd-p" style="position:relative"><div style="position:absolute;top:12px;right:12px;display:flex;gap:4px"><button class="ft-btn ft-bs ft-bo" style="color:#009DDD" onclick="FichateModule.editTurnoM(${t.id},'${(t.name||'').replace(/'/g,"\\'")}','${(t.start_time||'09:00:00').substring(0,5)}','${(t.end_time||'15:00:00').substring(0,5)}','${t.daily_hours||7.5}','${t.break_time||''}','${t.color||'#009DDD'}')">✏️</button><button class="ft-btn ft-bs ft-bo" style="color:#ef4444" onclick="FichateModule.delTurno(${t.id},'${(t.name||'').replace(/'/g,"\\'")}')">✕</button></div><div style="font-weight:700;font-size:16px;margin-bottom:8px;color:${t.color||'#009DDD'}">${t.name}</div><div style="font-size:14px;color:#6b7280;line-height:2"><div>📅 ${(t.start_time||'').substring(0,5)} - ${(t.end_time||'').substring(0,5)}</div><div>⏱ ${t.daily_hours||'7.5'}h/día</div>${t.break_time?`<div>☕ Pausa: ${t.break_time}</div>`:''}<div>👥 ${this.emps.filter(e=>e.schedule===t.name).length} empleados</div></div></div>`).join('')}</div>`}
